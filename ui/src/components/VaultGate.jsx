...
const inCls =
  `${FIELD_CLS} w-full`
const btnCls =
  'w-full mt-4 px-2.5 py-1.5 rounded-control bg-accent border border-accent text-on-accent text-copy font-medium disabled:opacity-50'
const cancelCls =
  'w-full mt-2 px-2.5 py-1.5 rounded-control border border-border text-field-txt text-copy bg-transparent disabled:opacity-50'
...
function Setup({ onDone }) {
  ...
  return (
    <Screen>
      <div className="text-copy font-semibold mb-5">Bloxsmith</div>
      <h1 className="text-copy font-semibold mb-2">Create your vault</h1>
      <p className="text-note leading-relaxed text-muted mb-4">
...
function Unlock({ onDone }) {
  ...
  return (
    <Screen>
      <div className="text-copy font-semibold mb-5">Bloxsmith</div>
      <h1 className="text-copy font-semibold mb-2">Unlock vault</h1>
      <p className="text-note leading-relaxed text-muted mb-4">Enter your passphrase to decrypt your saved tenant keys.</p>
      <label className="block text-note text-dim mb-1" htmlFor="vu-pass">Passphrase</label>
      <input
        id="vu-pass"
        className={inCls}
        type="password"
      ...
      <div className="text-center mt-3.5">
        <button
          className="bg-transparent border-none text-dim text-note underline underline-offset-2"
      ...
      {confirmReset && (
        <div className="mt-3 p-2.5 border border-crit/40 rounded-control bg-crit/10" role="dialog" aria-label="Confirm vault reset">
          <div className="text-note font-semibold mb-1">Reset vault?</div>
          <div className="text-note text-muted leading-relaxed mb-2">
            Permanently deletes the vault and all stored keys. No recovery. You will set a new passphrase.
          </div>
          <div className="flex gap-2">
            <button
              className="px-2.5 py-1 rounded-control border border-crit text-crit text-note"
              onClick={async () => {
            ...
            <button className="px-2.5 py-1 rounded-control border border-border text-field-txt text-note" autoFocus onClick={() => setConfirmReset(false)}>
              Cancel
            </button>
          </div>
        </div>
...
function FirstTenant({ onDone }) {
  ...
  return (
    <Screen>
      <div className="text-copy font-semibold mb-5">Bloxsmith</div>
      <h1 className="text-copy font-semibold mb-2">Add your first connection</h1>
      <p className="text-note leading-relaxed text-muted mb-4">
        Paste an Infoblox API key. The key is encrypted in your vault, and the connection is named automatically from
        its CSP account.
      </p>
      <label className="block text-note text-dim mb-1" htmlFor="vat-key">Infoblox API key</label>
      <input
        id="vat-key"
        className={inCls}
        type="password"
        ...
        autoFocus
      />
      <label className="block text-note text-dim mt-3 mb-1" htmlFor="vat-name">Name (optional)</label>
      <input
        id="vat-name"
        className={inCls}
        value={label}
      ...
      />
      <label className="block text-note text-dim mt-3 mb-1" htmlFor="vat-groq">Groq API key (optional — for AI query box)</label>
      <input
        id="vat-groq"
        className={inCls}
        type="password"
