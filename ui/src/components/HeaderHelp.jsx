    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 px-4" onClick={onClose} onKeyDown={onKeyDown}>
      ...
      >
        <div className="flex items-center mb-4">
          <h2 id="hh-title" className="text-copy font-semibold">What these controls do</h2>
        ...
        <dl className="m-0">
          {Object.entries(CONTROL_HELP).map(([id, { label, what }]) => (
            <div key={id} className="mb-3 last:mb-0">
              <dt className="text-note font-semibold text-field-txt">{label}</dt>
              <dd className="m-0 mt-0.5 text-note leading-relaxed text-muted">{what}</dd>
            </div>
          ))}
        </dl>
      </div>
