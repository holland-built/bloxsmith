...
export function BrandLogoImg({ domain, className, style, onClick, title, onSrcChange, bust }) {
  ...
  // The callback lives in a ref so the dependency list can be honest. Adding
  // `onSrcChange` to the deps directly would refire this on every render where
  // the parent passes an inline arrow — which is every render, for the usual
  // `onSrcChange={(s) => setX(s)}` call — turning a notify-on-change into a
  // notify-always. The ref keeps the effect keyed on `src` alone, which is what
  // it was always meant to be, while always calling the LATEST callback rather
