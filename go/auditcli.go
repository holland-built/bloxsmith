...
func auditCLIUsage() {
	println("usage: bloxsmith audit verify [options]")
	println()
	println("  Verifies the tamper-evident audit chain with nothing running. Read-only:")
	println("  it never creates a key and never reseals the chain.")
	println()
	println("  --log PATH         the audit_log.jsonl to check")
	println("                     (default: alongside the vault this install would use)")
	println("  --trust-dir DIR    directory holding audit.key and the sealed head")
	println("                     (default: $AUDIT_TRUST_DIR, else the per-user config dir)")
	println("  --key HEX          the HMAC key directly, instead of a key file ($AUDIT_KEY)")
	println("  --key-file PATH    a file holding the hex key ($AUDIT_KEY_FILE)")
	println("  --json             machine-readable result on stdout")
	println()
	println("  Exit codes: 0 intact, 1 tampered, 2 could-not-verify, 3 bad usage.")
	println("  2 is not a milder 1. It means the chain was NOT checked, so nothing")
	println("  is claimed about it either way.")
}
