...
- **Axur Supplier Risk**: security problems Axur found in the suppliers you depend on, worst first. Leaked staff logins, exposed services, dark-web mentions. This is the one panel on the tab that is not Infoblox data. Axur is a separate vendor with its own subscription and its own key, which goes in one of two places: the encrypted vault, under **⋯ Settings**, or the \`AXUR_API_KEY\` environment variable. A key in the vault wins over one in the environment, the same way the AI key behaves. The key belongs to the installation, not to a tenant, so switching Infoblox accounts does not change it. "Findings" counts the kinds of problem found for that supplier, not how many of each. A dash where the total goes means nothing was counted, and the panel says which reason applies: no key configured, the vault is locked, the account code could not be determined, or the read failed. An empty list reads as "no suppliers monitored in Axur", which is different news from "no problems found".
