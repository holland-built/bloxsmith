...
- **Rollback:** manual only — there is no automatic image rollback. `docker-compose.yml` defines no healthcheck and the updater never touches the Docker socket (see `go/apply.go`: it only swaps its own binary). To revert, pin the previous tag or digest in `docker-compose.yml` and run `docker compose up -d`; the `noc-vault` volume is untouched by the swap, so tenant keys survive it.
