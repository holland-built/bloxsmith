...
| `AXUR_CUSTOMER_KEY` |         | _(discovered)_           | Axur's short code for your account, which its supplier endpoints take as a path segment. Normally leave it unset: Bloxsmith asks Axur for it, and caches the answer. Set it when the panel reports that it could not work out which account to read, which happens when one login can see several Axur accounts. Bloxsmith refuses to guess between them rather than risk reading another company's suppliers |
